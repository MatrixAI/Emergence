...
To clarify, `nix-instantiate` is a function that translates the arguments attribute set (produced by nix expression) into a store derivation.
...
The `output` is the path of the resulting store derivation computed from the *inputs*, not from the derived output.
It is generated using: `makePath("output:out", hashDrv(d), name)` where `d` is the store derivation derived from the nix expression assuming `output=""`.
